...
def subscribe_to_calendly_webhook(
    callback_url: str, events: list, scope: str, organization: str
):
    """
    Subscribe to Calendly webhook.

    Args:
        callback_url (str): The URL where Calendly will send webhook events.
        events (list): List of events to subscribe to (e.g., ["invitee.created"]).
        scope (str): The scope of the webhook subscription (e.g., "organization", "user", or "group").
        organization (str): The unique reference to the organization.

    Returns:
        dict: The response from Calendly API.
    """
    url = f"{CALENDLY_API_BASE_URL}/webhook_subscriptions"
    headers = {
    ...
    if response.status_code == 201:
        ...
        raise HTTPException(
            ...
            detail=f"Failed to subscribe to Calendly webhook: {response.text}",
        )


@app.post("/api/calendly/subscribe")
async def calendly_subscribe(
    callback_url: str = "https://localhost:8000/api/calendly/webhook",  # Replace with your actual callback URL
    user: str = "https://api.calendly.com/saqibfarooq",  # Replace with your actual organization URL
):
    """
    Subscribe to Calendly webhook.
    """
    try:
        subscription_response = subscribe_to_calendly_webhook(
            callback_url=callback_url,
            events=["invitee.created"],  # Add other events if needed
            scope="user",
            organization=user,
        )
        return JSONResponse(
...
@router.post("/api/calendly/webhook")
async def calendly_webhook(
    ...
    This endpoint processes an invitee.created event and records meetings for all guests.
    """
    payload = await request.json()

    # Confirm event type is invitee.created
    event_type = payload.get("event")
    if event_type == "invitee.created":
        # Extract meeting details from payload
        scheduled_event = payload.get("payload", {}).get("scheduled_event", {})
        meeting_url = payload.get("payload", {}).get(
            "event"
        )  # Meeting URL from webhook payload
        scheduled_for = scheduled_event.get("start_time")

        # Extract guest emails from the scheduled event details
        event_guests = scheduled_event.get("event_guests", [])
        guest_emails = [
            guest.get("email") for guest in event_guests if guest.get("email")
        ]

        # Also add the primary invitee email if not already included
        primary_email = payload.get("payload", {}).get("email")
        if primary_email and primary_email not in guest_emails:
            guest_emails.append(primary_email)

        # Query the Users table for all emails provided in the payload.
        users = db.query(User).filter(User.email.in_(guest_emails)).all()

        # Process each user found.
        for user in users:
            # Create or fetch an existing session for the user.
            session = db.query(Session).filter(Session.user_id == user.user_id).first()
            if not session:
            ...
            # Create a meeting record for the user.
            new_meeting = Meeting(
